<script lang="ts">
  ...
  function onClickOutside(event: { target: { closest: (arg0: string) => any; }; }) {
...
<div class="fixed top-0 left-0 z-50 w-full header">
  <div class="w-screen bg-transparent">
    <nav
      ...
      <div
        class="absolute top-[50vw-100] left-0 z-100 right-0 flex-col grid place-items-center justify-center items-center h-2/3  md:h-full md:bg-transparent md:flex md:flex-row md:items-center md:justify-center space-x-0 md:space-x-4 md:mx-auto   {showMenu
          ? 'flex'
          : 'hidden'}">
        <div
          class="font-larger flex flex-col py-6 w-[80vw] h-[60vh] items-center justify-center md:flex-row bg-[blur] bg-stone-800/50 md:bg-transparent md:space-x-4 rounded-xl">
